...
serve(async (req) => {
  ...
  try {
    ...
    if (circleIds.length) {
      ...
      circleNames = (cs || []).map((c: { emoji: string | null; name: string }) =>
        `${c.emoji ?? ""} ${c.name}`.trim()
      );
    }
    ...
    const meetingsText = (meetings || []).length
      ? (meetings || []).map((m: { meeting_date: string; place: string | null; notes: string | null }) =>
    ...
    const brief = ((data.content || []) as Array<{ type: string; text?: string }>)
