...
| **A file** (merged) | ✓ | One picker card, accepts CSV / TSV / XLSX / images. `gatherFileCandidates()` in `src/lib/import/sources/file-source.ts` sniffs MIME and dispatches: spreadsheets go through `parseSpreadsheetFile`, Excel files convert to CSV via lazy-loaded SheetJS first, images go to `gatherPhotoOcrCandidates`. Per-row source ids in the DB stay accurate: 'spreadsheet' vs 'photo_ocr'. PDFs throw a friendly "coming soon" message. `parseSpreadsheetFile` does heuristic column detection across the full Person schema (`name`, `email`, `phone`, `company`, `title`, `how_we_met`, `where_when`, `physical_description`, `misc_notes`, `important_info`, `known_people_notes`) so a column called "How I met them" lands directly on `Person.how_we_met` instead of being smushed into a single notes blob. Truly unmapped columns become per-row `context` for the AI ranker. |
